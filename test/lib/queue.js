...
tap.test('Calls provider `subscribe` method once when first listener is added', (t) => {
  const q = new Queue({ provider: 'amqp' });
  q.provider.subscribe = sinon.stub();
  q.on('message', noop => noop);
  q.on('message', noop => noop);

  // Defer this until next event loop, since `subscribe` should also be called on next tick
  setImmediate(() => {
    t.ok(q.provider.subscribe.called, 'Provider `subscribe` not called.');
    t.notOk(q.provider.subscribe.calledTwice, 'Provider `subscribe` called more than once.');
    t.end();
  });
});
...
tap.test('Calls provider `unsubscribe` method once when last listener is removed', (t) => {
  const q = new Queue({ provider: 'amqp' });
  q.provider.subscribe = sinon.stub();
  q.provider.unsubscribe = sinon.stub();
  q.on('message', noop => noop);
  q.on('message', noop => noop);
  ...
  setImmediate(() => {
    ...
    t.ok(q.provider.unsubscribe.called, 'Provider `unsubscribe` not called.');
    t.notOk(q.provider.unsubscribe.calledTwice, 'Provider `unsubscribe` called more than once.');
    t.end();
  });
});
...
tap.test('Does not unsubscribe from provider if "removeListener" event name is not "message"', (t) => {
  ...
  setImmediate(() => {
    ...
    t.notOk(q.provider.unsubscribe.called, 'Provider `unsubscribe` should not have been called.');
    t.end();
  });
});
...
tap.test('Forwards message publishing to the provider', (t) => {
  ...
  q.provider.publish = sinon.stub();
  q.publish(expectedMessage);
  t.ok(q.provider.publish.called, 'Provider `publish` not called.');
  t.equal(q.provider.publish.getCall(0).args[0], expectedMessage);
  t.end();
});
...
tap.test('Forwards message ack to the provider', (t) => {
  ...
  q.provider.ack = sinon.stub();
  q.ack(expectedMessageId);
  t.ok(q.provider.ack.called, 'Provider `ack` not called.');
  t.equal(q.provider.ack.getCall(0).args[0], expectedMessageId);
  t.end();
});

tap.test('Forwards queue close to the provider', (t) => {
  const q = new Queue({ provider: 'amqp' });
  q.provider.close = sinon.stub();
  q.close();
  t.ok(q.provider.close.called, 'Provider `close` not called.');
  t.end();
});
